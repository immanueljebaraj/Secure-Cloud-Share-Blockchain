// src/app/owner/OwnerAudit.jsx
import { useEffect, useState } from 'react';
import { fetchAuditLogs }      from '../../api/audit';

const OWNER_ID = 1;

const ACTION_TYPES = ['ALL', 'UPLOAD', 'DOWNLOAD', 'DELETE', 'APPROVE', 'REJECT', 'REQUEST'];
...
export default function OwnerAudit() {
  ...
  const [tooltip, setTooltip] = useState(null); // tx hash tooltip

  const load = async () => {
    setLoading(true);
    try {
      const data = await fetchAuditLogs();
      // Show all logs relevant to this owner (all or filtered by userId)
      setLogs(data);
    } catch (err) {
  ...
  const actionClass = (action) => `audit-action-badge ${action}`;
      ...
      <div className="page-header">
        <div className="page-header-left">
          ...
          <p className="page-subtitle">
            Immutable blockchain record of all file system events
          </p>
        </div>
      ...
      <div className="audit-filters">
        <div className="search-input-wrap" style={{ maxWidth: 280 }}>
          ...
          <input
            ...
            placeholder="Search by file, user, tx hash…"
            value={search}
            ...
            {loading ? (
              ...
              filtered.map((l) => (
                <tr key={l.id}>
                  ...
                  <td>
                    <span className={actionClass(l.action)}>{l.action}</span>
                  </td>
                  <td>
                    <span className="mono" style={{ color: 'var(--text-secondary)' }}>#{l.fileId}</span>
                  </td>
                  <td>
                    <span className="mono" style={{ color: 'var(--text-secondary)' }}>{l.userId}</span>
                  </td>
                  <td>
                  ...
                  <td>
                    <span
                      className="tx-hash"
                      title={l.txHash ?? 'No transaction hash'}
                      onClick={() => {
                        if (l.txHash) navigator.clipboard?.writeText(l.txHash);
                      }}
                    >
                      {l.txHash ? shortHash(l.txHash) : (
