...
const VENDOR_ID     = 2;
const ACTION_TYPES  = ['ALL', 'UPLOAD', 'DOWNLOAD', 'REQUEST'];
...
export default function VendorAudit() {
  ...
  const load = async () => {
    ...
    try {
      ...
      // Show only this vendor's actions
        ...
        <table className="data-table">
          ...
          <tbody>
            {loading ? (
              ...
              filtered.map((l) => (
                <tr key={l.id}>
                  <td><span className="mono" style={{ color:'var(--text-muted)' }}>#{l.id}</span></td>
                  <td>
                    <span className={`audit-action-badge ${l.action}`}>{l.action}</span>
                  </td>
                  <td><span className="mono" style={{ color:'var(--text-secondary)' }}>#{l.fileId}</span></td>
                  <td><span className="audit-time">{formatDateTime(l.createdAt)}</span></td>
                  <td>
                    <span
                      className="tx-hash"
